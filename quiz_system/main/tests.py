...
class AnswerTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.answer1 = create_answer(answer="An attempt to explain a phenomenon", question=cls.question1,
                                    is_correct=True)
...
class QuizTakerResponseTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.answer2 = create_answer(answer="Just humanity ego in reasoning things out", question=cls.question1)
        cls.answer3 = create_answer(answer="It's an art", question=cls.question2, is_correct=False)
        cls.answer4 = create_answer(answer="When the sun is overshadowed by moon", question=cls.question3,
                                    is_correct=False)
        ...
        cls.stud_response1 = create_quiz_taker_response(quiztaker=cls.quiz_taker1, question=cls.question1,
                                                        answer=cls.answer1)
        cls.stud_response2 = create_quiz_taker_response(quiztaker=cls.quiz_taker1, question=cls.question1,
                                                        answer=cls.answer2)
        cls.stud_response3 = create_quiz_taker_response(quiztaker=cls.quiz_taker2, question=cls.question1,
                                                        answer=cls.answer1)

    def setUp(self):
        self.stud_response1 = QuizTakerResponse.objects.get(quiztaker=self.quiz_taker1, question=self.question1,
                                                            answer=self.answer1)
        self.stud_response2 = QuizTakerResponse.objects.get(quiztaker=self.quiz_taker1, question=self.question1,
                                                            answer=self.answer2)
        self.stud_response3 = QuizTakerResponse.objects.get(quiztaker=self.quiz_taker2, question=self.question1,
                                                            answer=self.answer1)
    ...
    def test_response_to_quiz_subject_not_in_interests(self):
        with self.assertRaises(Quiz.DoesNotExist):
            self.stud_response4 = QuizTakerResponse.objects.create(quiztaker=self.quiz_taker1, question=self.question2,
                                                                   answer=self.answer3)
...
class QuizTakeViewTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.user = create_custom_user(username="studentias", is_student=True, password="password123456")
...
class QuizResultsViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.quiz_taker_response = create_quiz_taker_response(quiztaker=self.quiz_taker, question=self.question1,
                                                              answer=self.answer1)

    def test_final_post_results_redirect(self):
        # self.client.login(username=self.user.username, password='password123456')
